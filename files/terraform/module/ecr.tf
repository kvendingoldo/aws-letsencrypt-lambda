...
resource "aws_secretsmanager_secret_version" "image_registry" {
  ...
  secret_id = aws_secretsmanager_secret.image_registry[0].id
  secret_string = jsonencode({
    username : var.ecr_proxy_username,
    accessToken : var.ecr_proxy_access_token
  })
}

resource "aws_ecr_pull_through_cache_rule" "lambda_proxy" {
